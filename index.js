...
io.sockets.on('connection', function(socket) { //socket code
    ...
    socket.on('message',function(obj){
        ...
        try{
            io.sockets.in(`game-${obj.gameId}`).emit('onMessage',{playerId:obj.playerId,playerName:obj.playerId.split('-')[2],playerAvatar:`images/avatars/${obj.playerId.split('-')[4].split('~')[0]}.png`,message:obj.message.replace('\n','<br>')});
        }
        catch(ex){
